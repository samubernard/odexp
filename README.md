...
If *file* is not given, *odexp* will take the dynamical system from the standard input.
When option **&#45;o** is present, system parameters, initial conditions and options are loaded from file *parameterfile*.

*odexp* uses the *GNU Scientific Library* (GSL) for numerical integration of ODEs and DDEs and their
...
> Print dataset for particle *id*. If *id* is missing, print stats dataset
> in population mode or particle in single mode.

\* \[*msg*]

> Snapshot of current simulation and parameter values with optional *msg*.
...
> Steady state must have been computed with **ms**.
...
> Make parameter with index *ind* or name *par* the current parameter, and set its value to *val*.
> When val is missing, the parameter value is unchanged.

Q, q \[*msg*, *C^w*]

> Quit and make a snapshot of the paramters. An optional message *msg* can be added to **Q** or **q**.
...
> Set value of initial condition of variable with index *i* or name *var* to *val*.
...
> Change to last initial conditions, same as **il** but do not run simulation.
...
> and z-axis to *k* (variable *z*).
> Set variable to T or index -1 for time.
> **2** takes only the first two arguments, and the **3** takes the three arguments
...
*name* must be a valid C variable name.
*value* must be a constant number; by default a double, but can be
an integer with attribute *int* or *long*.
...
Implicit initial condition. If *var* is a dynamical variable, the declaration

> PAR var\_0 0.5

declares the parameter *var\_0*, sets it to 0.5 and implicitly declares the initial condition INIT *var*
*var\_0*.
...
for the lifetime of the particle. Use *ATBIRTH* and *ATREPLI* to specify particle-dependent expressions.
...
Dynamical variable *name* is declared as d*name*/dt followed by = and the *rhs* of the equation
...
For each equation D/DT, there must be an INIT with the corresponding *name*.
...
where *nrows* *ncols* are the number of rows and columns in the file *filename*.
*filename* is a text file containing a space delimited array of doubles.
...
*%C*
Coupling term.
This is of the form PSI\[i] = 1/POP\_SIZE\*sum\_{j=1}^POP\_SIZE *phi*(x\[j],x\[i]), where *phi* is a function of two variables. The declaration is
...
*%M*
Mean field.
This is of the form MF = 1/POP\_SIZE\*sum(j=1) *phi*(x\[j]), where *phi* depend only on one variable.
...
When the rhight-hand side is equal to *lrexp* in a coupling declaration, an adaptative rank P expansion
is used to approximate the coupling function g given in the attribute *fun* over the variable given
in attribute *var*.
